function addPrCommitLinksAutoEmbed() {
  ...
  function ref(initialValue) {
    ...
    return {
      ...
      set value(newValue) {
        ...
        effects.forEach((effect) => effect(newValue, oldValue));
      },
      watchEffect(effect) {
        effects.add(effect);
  ...
  return (async function (ctx, domUtils) {
    ...
    observeIsCommentEditing();

    addAutoEmbedSectionInSidebar({
      onClickSync: syncPrCommits,
      onCheckPrCommitsIncluded: checkPrCommitsIncluded,
    });

    watches.isCommentEditing.watchEffect(function updateAutoEmbedSection() {
      addAutoEmbedSectionInSidebar({
        onClickSync: syncPrCommits,
        onCheckPrCommitsIncluded: checkPrCommitsIncluded,
      });
    ...
    function observeIsCommentEditing() {
      let observer = null;
        ...
        observer = new MutationObserver((mutations) => {
          mutations.forEach((mutation) => {
            if (mutation.attributeName === "class") {
              const isEditing =
                mutation.target.classList.contains("is-comment-editing");
              if (isEditing) {
                watches.isCommentEditing.value = true;
              } else {
                watches.isCommentEditing.value = false;
              }
            }
          });
        });
...
function addAutoEmbedSectionInSidebar({
  onClickSync,
